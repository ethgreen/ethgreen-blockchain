...
if [ "$(uname -m)" = "armv7l" ]; then
  echo ""
  echo "WARNING:"
  echo "The ETHgreen Blockchain requires a 64 bit OS and this is 32 bit armv7l"
  echo "For more information, see"
  echo "https://github.com/ethgreen/ethgreen-blockchain/wiki/Raspberry-Pi"
  echo "Exiting."
  exit 1
fi
# Get submodules
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with ethgreen
  ...
  # yum install python3 brings Python3.6 which is not supported by ethgreen
  cd ..
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "11" ]; then
  echo "ETHgreen requires Python version >= 3.7 and  < 3.11.0" >&2
...
echo ""
echo "ETHgreen blockchain install.sh complete."
echo "For assistance join us on Discord in the #support chat channel:"
echo "https://discord.gg/yEWaF6CQcA"
echo ""
echo "Try the Quick Start Guide to running ethgreen-blockchain:"
echo "https://github.com/ethgreen/ethgreen-blockchain/wiki/Quick-Start-Guide"
echo ""
echo "To install the GUI type 'sh install-gui.sh' after '. ./activate'."
echo ""
echo "Type '. ./activate' and then 'ethgreen init' to begin."
