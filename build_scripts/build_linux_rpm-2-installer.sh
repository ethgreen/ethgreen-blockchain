...
# Builds CLI only rpm
CLI_RPM_BASE="ethgreen-blockchain-cli-$ETHGREEN_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/ethgreen"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/ethgreen/"
ln -s ../../opt/ethgreen/ethgreen "dist/$CLI_RPM_BASE/usr/bin/ethgreen"
...
fpm -s dir -t rpm \
  ...
  --name ethgreen-blockchain-cli \
  --license Apache-2.0 \
  --version "$ETHGREEN_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "ETHgreen is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  --depends /usr/lib64/libcrypt.so.1 \
  .
# CLI only rpm done

cp -r dist/daemon ../ethgreen-blockchain-gui/packages/gui

# Change to the gui package
cd ../ethgreen-blockchain-gui/packages/gui || exit 1
...
PRODUCT_NAME="ethgreen"
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="ETHgreen Blockchain" \
  --config.rpm.packageName="ethgreen-blockchain"
...
GUI_RPM_NAME="ethgreen-blockchain-${ETHGREEN_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${ETHGREEN_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
