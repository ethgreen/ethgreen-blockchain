...
from ethgreen.plot_sync.delta import Delta
from ethgreen.plot_sync.receiver import Receiver
from ethgreen.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
from ethgreen.protocols import farmer_protocol, harvester_protocol
from ethgreen.protocols.pool_protocol import (
...
from ethgreen.server.server import ssl_context_for_root
...
from ethgreen.util.bech32m import decode_puzzle_hash
from ethgreen.util.byte_types import hexstr_to_bytes
from ethgreen.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from ethgreen.util.errors import KeychainProxyConnectionFailure
from ethgreen.util.hash import std_hash
from ethgreen.util.ints import uint8, uint16, uint32, uint64
...
from ethgreen.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'ethgreen keys generate' or open the UI."
        try:
        ...
        if "xeth_target_address" not in self.config:
            self.config = config["farmer"]
        if "xeth_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xeth_target_address" not in self.config or "xeth_target_address" not in self.pool_config:
            log.debug("xeth_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["xeth_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)
        self.donationwallet_target = self.constants.GENESIS_PRE_FARM_DONATIONWALLET_PUZZLE_HASH

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["xeth_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["xeth_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["xeth_target_address"] = pool_target_encoded
