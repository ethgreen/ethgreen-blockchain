...
from ethgreen.server.start_service import RpcInfo, Service, async_run
from ethgreen.types.peer_info import PeerInfo
from ethgreen.util.ethgreen_logging import initialize_service_logging
from ethgreen.util.config import load_config_cli, load_config
...
from ethgreen.wallet.wallet_node_api import WalletNodeAPI
...
async def async_main() -> int:
    ...
    if local_test is True:
        from ethgreen.simulator.block_tools import test_constants
...
def main() -> int:
    freeze_support()
    if os.getenv("ETHGREEN_INSTRUMENT_WALLET", 0) != 0:
        from ethgreen.util.task_timing import start_task_instrumentation, stop_task_instrumentation
        import atexit
