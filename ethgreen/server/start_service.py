...
from ethgreen.daemon.server import service_launch_lock_path
...
from ethgreen.cmds.init_funcs import ethgreen_full_version_str
from ethgreen.rpc.rpc_server import RpcApiProtocol, RpcServiceProtocol, start_rpc_server, RpcServer
from ethgreen.server.outbound_message import NodeType
from ethgreen.server.server import ETHgreenServer
from ethgreen.server.upnp import UPnP
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        self._log.info(f"ethgreen-blockchain version: {ethgreen_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        ethgreen_ca_crt, ethgreen_ca_key = ethgreen_ssl_ca_paths(root_path, self.config)
        ...
        self._server = ETHgreenServer(
            ...
            (ethgreen_ca_crt, ethgreen_ca_key),
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"ethgreen_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for ETHgreenServer to be closed")
