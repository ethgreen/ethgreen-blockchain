...
from ethgreen.util.default_root import DEFAULT_KEYS_ROOT_PATH
from ethgreen.util.file_keyring import FileKeyring
from ethgreen.util.misc import prompt_yes_no
...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ ethgreen passphrase set # all the cool kids are doing it!"

MASTER_PASSPHRASE_SERVICE_NAME = "ETHgreen Passphrase"
MASTER_PASSPHRASE_USER_NAME = "ETHgreen Passphrase"
...
def check_legacy_keyring_keys_present(keyring: LegacyKeyring) -> bool:
    ...
    from ethgreen.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def __init__(self, keys_root_path: Path = DEFAULT_KEYS_ROOT_PATH, force_legacy: bool = False):
        ...
        """
        from ethgreen.util.errors import KeychainNotSet
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from ethgreen.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'ethgreen passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase?")
            if response:
                from ethgreen.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'ethgreen passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from ethgreen.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    async def migrate_legacy_keyring_interactive(self) -> bool:
        ...
        """
        from ethgreen.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running

        # Let the user know about the migration.
        if not self.confirm_migration():
            print("Migration aborted, can't run any ethgreen commands.")
            return False
