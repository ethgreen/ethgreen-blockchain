...
from ethgreen.consensus.block_rewards import calculate_base_farmer_reward, calculate_base_donationwallet_reward
from ethgreen.data_layer.data_layer_wallet import DataLayerWallet
from ethgreen.pools.pool_wallet import PoolWallet
from ethgreen.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from ethgreen.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from ethgreen.types.blockchain_format.coin import Coin, coin_as_list
...
from ethgreen.util.byte_types import hexstr_to_bytes
from ethgreen.util.config import load_config
from ethgreen.util.errors import KeychainIsLocked
from ethgreen.util.ints import uint8, uint32, uint64, uint16
from ethgreen.util.keychain import bytes_to_mnemonic, generate_mnemonic
from ethgreen.util.path import path_from_root
from ethgreen.util.ws_message import WsRpcMessage, create_payload_dict
...
from ethgreen.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
from ethgreen.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
...
from ethgreen.wallet.notification_store import Notification
from ethgreen.wallet.outer_puzzles import AssetType
from ethgreen.wallet.puzzle_drivers import PuzzleInfo, Solver
from ethgreen.wallet.trade_record import TradeRecord
from ethgreen.wallet.trading.offer import Offer
from ethgreen.wallet.transaction_record import TransactionRecord
from ethgreen.wallet.util.address_type import AddressType, is_valid_address
...
from ethgreen.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "ethgreen_wallet"
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xeth_target_address")
        pool_target = config["pool"].get("xeth_target_address")
    ...
    async def create_new_wallet(self, request: Dict) -> EndpointResult:
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from ethgreen.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from ethgreen.util.bech32m import bech32_decode, convertbits
        from ethgreen.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSETHgreenConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xeth")
        ...
        key = None
        if asset_id is not None and asset_id != "xeth":
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xeth_coin_list = request.get("xeth_coins", None)
        xeth_coins = None
        if xeth_coin_list:
            xeth_coins = set([Coin.from_json_dict(xeth_coin) for xeth_coin in xeth_coin_list])
        xeth_change_target = request.get("xeth_change_target", None)
        if xeth_change_target is not None:
            if xeth_change_target[:2] == "xeth":
                xeth_change_ph = decode_puzzle_hash(xeth_change_target)
            else:
                xeth_change_ph = bytes32(hexstr_to_bytes(xeth_change_target))
        else:
            xeth_change_ph = None
        ...
        if mint_from_did:
            sb = await nft_wallet.mint_from_did(
                ...
                xeth_coins=xeth_coins,
                xeth_change_ph=xeth_change_ph,
            ...
            sb = await nft_wallet.mint_from_xeth(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def get_farmed_amount(self, request) -> EndpointResult:
        ...
        donationwallet_reward_amount = 0
        fee_amount = 0
        ...
        for record in tx_records:
            ...
            # ethgreen Network Code
            # Do not need to calculate the Community Rewards Amount To Wallet Card
            if( uint64(calculate_base_donationwallet_reward(height)) != uint64(record.amount) ):
        ...
        return {
            ...
            "donationwallet_reward_amount": donationwallet_reward_amount,
