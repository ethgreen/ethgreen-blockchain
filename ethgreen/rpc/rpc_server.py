...
from ethgreen.rpc.util import wrap_http_handler
from ethgreen.server.outbound_message import NodeType
from ethgreen.server.server import ETHgreenServer, ssl_context_for_client, ssl_context_for_server
...
from ethgreen.util.config import str2bool
...
from ethgreen.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> ETHgreenServer:
        ...
        # a property so as to be read only which allows ETHgreenServer to satisfy
        # Optional[ETHgreenServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        ethgreen.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSETHgreenConnection) -> None:
...
def default_get_connections(server: ETHgreenServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
