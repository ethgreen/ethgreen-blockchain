...
from ethgreen.cmds.passphrase_funcs import obtain_current_passphrase
from ethgreen.daemon.client import connect_to_daemon_and_validate
...
from ethgreen.util.errors import KeychainException
from ethgreen.util.file_keyring import MAX_LABEL_LENGTH
from ethgreen.util.ints import uint32
from ethgreen.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def generate_and_print():
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run ethgreen keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'ethgreen keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from ethgreen.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using ETHgreen.\n"
    )
