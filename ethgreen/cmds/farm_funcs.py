...
from ethgreen.cmds.cmds_util import get_any_service_client
...
from ethgreen.util.misc import format_bytes
from ethgreen.util.misc import format_minutes
from ethgreen.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total ethgreen farmed: {amounts['farmed_amount'] / units['ethgreen']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['ethgreen']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['donationwallet_reward_amount'] + amounts['pool_reward_amount']) / units['ethgreen']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'ethgreen start wallet' and 'ethgreen wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'ethgreen wallet show'")
    else:
        print("Note: log into your key using 'ethgreen wallet show' to see rewards for each key")
