...
from ethgreen.daemon.client import acquire_connection_to_daemon
from ethgreen.util.config import load_config
from ethgreen.util.errors import KeychainMaxUnlockAttempts
from ethgreen.util.keychain import Keychain, supports_os_passphrase_storage
from ethgreen.util.keyring_wrapper import KeyringWrapper, DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from ethgreen.util.misc import prompt_yes_no
from ethgreen.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other ETHgreen processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("ETHGREEN_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'ethgreen passphrase set' or 'ethgreen passphrase remove' to update or remove your passphrase")
        sys.exit(1)
