...
from ethgreen.cmds.plotnft import validate_fee
...
from ethgreen.cmds.cmds_util import execute_with_wallet
...
@click.option(
    "-ma",
    "--min_coin_amount",
    help="Ignore coins worth less then this much XETH or CAT units",
    type=str,
    required=False,
    default="0",
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.XETH}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of XETH/CATs for XETH/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XETH.",
    type=str,
    default="0",
    show_default=True,
