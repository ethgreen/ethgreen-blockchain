...
from ethgreen.consensus.coinbase import create_puzzlehash_for_pk
from ethgreen.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_ethgreen_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from ethgreen.util.bech32m import encode_puzzle_hash
from ethgreen.util.config import (
    create_default_ethgreen_config,
    initial_config_file,
    load_config,
    save_config,
...
from ethgreen.util.ssl_check import (
...
from ethgreen.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from ethgreen.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'ethgreen keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xeth_target_address" not in config["farmer"]
    stop_searching_for_pool = "xeth_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xeth_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xeth_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "xeth_target_address" not in config["farmer"]:
        print(
            f"Setting the xeth destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["xeth_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["xeth_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xeth_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xeth_target_address" not in config["pool"]:
        print(f"Setting the xeth destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["xeth_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["xeth_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xeth_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the ETHGREEN destination addresses, edit the `xeth_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    ethgreen_ca_crt, ethgreen_ca_key = get_ethgreen_ca_crt_key()
    ethgreen_ca_crt_path = ca_dir / "ethgreen_ca.crt"
    ethgreen_ca_key_path = ca_dir / "ethgreen_ca.key"
    write_ssl_cert_and_key(ethgreen_ca_crt_path, ethgreen_ca_crt, ethgreen_ca_key_path, ethgreen_ca_key)
    ...
    ethgreen_ca_crt, ethgreen_ca_key = get_ethgreen_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, ethgreen_ca_crt, ethgreen_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                ethgreen_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return ethgreen_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def ethgreen_version_number() -> Tuple[str, str, str, str]:
...
def ethgreen_minor_release_number():
    res = int(ethgreen_version_number()[2])
    print(f"Install release number: {res}")
    return res


def ethgreen_full_version_str() -> str:
    major, minor, patch, dev = ethgreen_version_number()
...
def ethgreen_init(
    ...
    """
    if os.environ.get("ETHGREEN_ROOT", None) is not None:
        print(
            f"warning, your ETHGREEN_ROOT is set to {os.environ['ETHGREEN_ROOT']}. "
            f"Please unset the environment variable and run ethgreen init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Ethgreen directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if ETHGREEN_ROOT is set, or if user has run ethgreen init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'ethgreen keys show --show-mnemonic-seed'")
