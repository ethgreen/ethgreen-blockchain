...
from ethgreen.ssl.create_ssl import generate_ca_signed_cert, get_ethgreen_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if "xeth_target_address" not in config["farmer"]:
        print(f"Setting the xeth destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "xeth_target_address" not in config["pool"]:
        print(f"Setting the xeth destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    ethgreen_ca_crt_path.write_bytes(ethgreen_ca_crt)
    ethgreen_ca_key_path.write_bytes(ethgreen_ca_key)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if ethgreen_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return ethgreen_init(root_path)
...
def ethgreen_init(root_path: Path):
