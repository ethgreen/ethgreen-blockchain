...
from ethgreen.util.db_version import set_db_version
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xeth_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xeth_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xeth_target_address") or all_targets[-2] == config["pool"].get(
                    "xeth_target_address"
                ):
        ...
        if "xeth_target_address" not in config["farmer"]:
            print(
                f"Setting the xeth destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if updated_target:
            print(
                f"To change the XETH destination addresses, edit the `xeth_target_address` entries in"
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(ethgreen_ca_crt_path, ethgreen_ca_crt, ethgreen_ca_key_path, ethgreen_ca_key, overwrite=overwrite)
...
def init(
    ...
    if create_certs is not None:
        ...
        return ethgreen_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def ethgreen_init(
    ...
    *,
    ...
    """
    ethgreen_root = os.environ.get("ETHGREEN_ROOT", None)
    if ethgreen_root is not None:
        print(f"ETHGREEN_ROOT is set to {ethgreen_root}")
