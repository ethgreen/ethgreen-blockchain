...
from ethgreen.consensus.block_root_validation import validate_block_merkle_roots
...
from ethgreen.full_node.mempool_check_conditions import get_name_puzzle_conditions, mempool_check_time_locks
...
from ethgreen.util.generator_tools import tx_removals_and_additions
    ...
    *,
    ...
    if height > 0:
        ...
        donationwallet_coin = create_donationwallet_coin(
            ...
            calculate_base_donationwallet_reward(prev_transaction_block.height),
            constants.GENESIS_CHALLENGE,
        )
        # Adds the previous block
        expected_reward_coins.add(pool_coin)
        expected_reward_coins.add(farmer_coin)
        expected_reward_coins.add(donationwallet_coin)
        ...
        if prev_transaction_block.height > 0:
            ...
            while not curr_b.is_transaction_block:
                ...
                expected_reward_coins.add(
                        create_donationwallet_coin(
                        curr_b.height,
                        constants.GENESIS_PRE_FARM_DONATIONWALLET_PUZZLE_HASH,
                        calculate_base_donationwallet_reward(curr_b.height),
                        constants.GENESIS_CHALLENGE,
                    )
                )
