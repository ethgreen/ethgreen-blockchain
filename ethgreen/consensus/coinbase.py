...
from ethgreen.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_pk
...
def donationwallet_parent_id(block_height: uint32, genesis_challenge: bytes32) -> uint32:
    return bytes32(genesis_challenge[16:] + block_height.to_bytes(16, "big"))
...
def create_donationwallet_coin(block_height: uint32, puzzle_hash: bytes32, reward: uint64, genesis_challenge: bytes32):
    parent_id = donationwallet_parent_id(block_height, genesis_challenge)
    return Coin(parent_id, puzzle_hash, reward)