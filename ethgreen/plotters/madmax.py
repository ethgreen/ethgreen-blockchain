...
from ethgreen.plotting.create_plots import resolve_plot_keys
from ethgreen.plotters.plotters_util import (
    run_plotter,
    run_command,
...
def get_madmax_exec_venv_path(ksize: int = 32) -> Optional[Path]:
    ...
    if not venv_bin_path:
        return None
    madmax_exec = "ethgreen_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the ethgreen_plot_k34 executable for k-sizes > 32
...
def plot_madmax(args, ethgreen_root_path: Path, plotters_root_path: Path):
    ...
    try:
        asyncio.run(run_plotter(ethgreen_root_path, args.plotter, call_args, progress))
    except Exception as e:
