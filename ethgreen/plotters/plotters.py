...
from ethgreen.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from ethgreen.plotters.chiapos import get_chiapos_install_info, plot_ethgreen
from ethgreen.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in ETHGREEN_ROOT.
    ethgreen_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within ETHGREEN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("ethgreen plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, ethgreen_plotter_options, "chiapos", "Create a plot with the default ethgreen plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'ethgreen plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        ...
        plot_ethgreen(args, ethgreen_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, ethgreen_root_path, root_path)
    elif args.plotter.startswith("bladebit"):
        plot_bladebit(args, ethgreen_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(ethgreen_root_path)
