...
from ethgreen.simulator.simulator_protocol import FarmNewBlockProtocol, GetAllCoinsProtocol, ReorgProtocol
...
class FullNodeSimulator(FullNodeAPI):
    ...
    async def farm_new_transaction_block(
        ...
        async with self.full_node._blockchain_lock_high_priority:
            ...
            self.log.info(self.full_node.constants.GENESIS_PRE_FARM_DONATIONWALLET_PUZZLE_HASH)
            current_blocks = await self.get_all_full_blocks()
            ...
            more = self.bt.get_consecutive_blocks(
                ...
                donationwallet_reward_puzzle_hash=self.full_node.constants.GENESIS_PRE_FARM_DONATIONWALLET_PUZZLE_HASH,
    ...
    async def process_blocks(self, count: int, farm_to: bytes32 = bytes32([0] * 32)) -> int:
        ...
        Consider `.farm_blocks()` or `.farm_rewards()` if the goal is to receive XETH at
        an address.
