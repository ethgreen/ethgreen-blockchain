...
from ethgreen.cmds.init_funcs import create_all_ssl
from ethgreen.consensus.coinbase import create_puzzlehash_for_pk
from ethgreen.daemon.server import WebSocketServer, daemon_launch_lock_path
...
from ethgreen.simulator.start_simulator import async_main as start_simulator_main
from ethgreen.types.blockchain_format.sized_bytes import bytes32
from ethgreen.util.bech32m import encode_puzzle_hash
from ethgreen.util.config import create_default_ethgreen_config, load_config, save_config
from ethgreen.util.ints import uint32
from ethgreen.util.keychain import Keychain
from ethgreen.util.lock import Lockfile
...
def create_config(
    ...
    # create ethgreen directories
    create_default_ethgreen_config(ethgreen_root)
    create_all_ssl(
        ethgreen_root,
    ...
    # load config
    config = load_config(ethgreen_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(fingerprint), "txeth")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(ethgreen_root, "config.yaml", config)
    return config


async def start_simulator(ethgreen_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=ethgreen_root)
    await service.start()
...
async def get_full_ethgreen_simulator(
    automated_testing: bool = False, ethgreen_root: Optional[Path] = None, config: Optional[Dict[str, Any]] = None
) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int], None]:
    """
    A ethgreen root directory can be provided, otherwise a temporary one is created.
    This test can either be run in automated mode or not, which determines which mode block tools run in.
    This test is fully interdependent and can be used without the rest of the ethgreen test suite.
    Please refer to the documentation for more information.
    """
    # Create and setup temporary ethgreen directories.
    if ethgreen_root is None:
        ethgreen_root = Path(tempfile.TemporaryDirectory().name)
    ...
    crt_path = ethgreen_root / config["daemon_ssl"]["private_crt"]
    key_path = ethgreen_root / config["daemon_ssl"]["private_key"]
    ca_crt_path = ethgreen_root / config["private_ssl_ca"]["crt"]
    ca_key_path = ethgreen_root / config["private_ssl_ca"]["key"]
    with Lockfile.create(daemon_launch_lock_path(ethgreen_root)):
        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(ethgreen_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()  # type: ignore[no-untyped-call]

        async for simulator in start_simulator(ethgreen_root, automated_testing):
            yield simulator, ethgreen_root, config, mnemonic, fingerprint
