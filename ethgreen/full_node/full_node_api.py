...
import ethgreen.server.ws_connection as ws
from ethgreen.consensus.block_creation import create_unfinished_block
from ethgreen.consensus.block_record import BlockRecord
from ethgreen.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from ethgreen.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from ethgreen.full_node.fee_estimate import FeeEstimate, FeeEstimateGroup
from ethgreen.full_node.full_node import FullNode
from ethgreen.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from ethgreen.full_node.signage_point import SignagePoint
from ethgreen.full_node.fee_estimator_interface import FeeEstimatorInterface
...
from ethgreen.types.block_protocol import BlockInfo
...
from ethgreen.util.full_block_utils import header_block_from_block
from ethgreen.util.generator_tools import get_block_header, tx_removals_and_additions
...
class FullNodeAPI:
    ...
    async def request_peers(
        self, _request: full_node_protocol.RequestPeers, peer: ws.WSETHgreenConnection
    ) -> Optional[Message]:
    ...
    async def declare_proof_of_space(
        ...
        async with self.full_node.timelord_lock:
            ...
            if prev_b is None:
                ...
                donationwallet_ph = self.full_node.constants.GENESIS_PRE_FARM_DONATIONWALLET_PUZZLE_HASH
            else:
