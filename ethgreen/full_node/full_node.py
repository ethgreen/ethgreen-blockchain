...
from ethgreen.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from ethgreen.util.path import mkdir, path_from_root
...
class FullNode:
    ...
    def set_server(self, server: EthgreenServer):
        self.server = server
        dns_servers = []
        try:
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.ethgreen.net")
        try:
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSEthgreenConnection):
        """
    ...
    async def on_connect(self, connection: ws.WSEthgreenConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSEthgreenConnection):
    ...
    def remove_subscriptions(self, peer: ws.WSEthgreenConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSEthgreenConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSEthgreenConnection, List[FullBlock]]] = asyncio.Queue(
