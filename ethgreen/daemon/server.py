...
from ethgreen.cmds.init_funcs import check_keys, ethgreen_init
...
from ethgreen.util.ethgreen_logging import initialize_logging
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("ethgreen daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with ETHGREEN_ROOT: {os.environ['ETHGREEN_ROOT']}")
    ...
    if service_command == "ethgreen_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
