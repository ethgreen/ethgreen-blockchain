...
from ethgreen.cmds.init_funcs import check_keys, ethgreen_init, ethgreen_full_version_str
from ethgreen.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from ethgreen.daemon.keychain_server import KeychainServer, keychain_commands
from ethgreen.daemon.windows_signal import kill
from ethgreen.plotters.plotters import get_available_plotters
from ethgreen.plotting.util import add_plot_directory
from ethgreen.server.server import ssl_context_for_root, ssl_context_for_server
from ethgreen.ssl.create_ssl import get_mozilla_ca_crt
from ethgreen.util.beta_metrics import BetaMetricsLogger
from ethgreen.util.ethgreen_logging import initialize_service_logging
from ethgreen.util.config import load_config
from ethgreen.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
...
from ethgreen.util.lock import Lockfile, LockfileError
from ethgreen.util.network import WebServer
from ethgreen.util.service_groups import validate_service
from ethgreen.util.setproctitle import setproctitle
from ethgreen.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting ETHgreen.")
    quit()


log = logging.getLogger(__name__)

service_plotter = "ethgreen_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "ethgreen": "ethgreen",
        "ethgreen_data_layer": "start_data_layer",
        "ethgreen_data_layer_http": "start_data_layer_http",
        "ethgreen_wallet": "start_wallet",
        "ethgreen_full_node": "start_full_node",
        "ethgreen_harvester": "start_harvester",
        "ethgreen_farmer": "start_farmer",
        "ethgreen_introducer": "start_introducer",
        "ethgreen_timelord": "start_timelord",
        "ethgreen_timelord_launcher": "timelord_launcher",
        "ethgreen_full_node_simulator": "start_simulator",
        "ethgreen_seeder": "start_seeder",
        "ethgreen_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of ETHgreen will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["ethgreen", "plotters", plotter]
    ...
    async def exit(self) -> None:
        ...
        log.info("ethgreen daemon exiting")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered ETHGREEN_ROOT
    os.environ["ETHGREEN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with ETHGREEN_ROOT: {os.environ['ETHGREEN_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in ethgreen_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    ethgreen_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("ethgreen_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"ethgreen-blockchain version: {ethgreen_full_version_str()}")
