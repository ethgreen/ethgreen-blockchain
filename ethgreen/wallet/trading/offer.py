...
from ethgreen.util.bech32m import bech32_decode, bech32_encode, convertbits
from ethgreen.util.ints import uint64
from ethgreen.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    match_puzzle,
    solve_puzzle,
...
from ethgreen.wallet.util.puzzle_compression import (
    compress_object_with_puzzles,
    decompress_object_with_puzzles,
    lowest_best_version,
)
from ethgreen.wallet.uncurried_puzzle import UncurriedPuzzle, uncurry_puzzle
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting XETH
        coins: List[Coin],
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "xeth" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
