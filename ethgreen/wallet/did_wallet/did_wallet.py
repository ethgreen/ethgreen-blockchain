...
from ethgreen.util.ints import uint64, uint32, uint8, uint128
...
from ethgreen.wallet.did_wallet.did_info import DIDInfo
...
from ethgreen.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
from ethgreen.wallet.coin_selection import select_coins
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSETHgreenConnection):
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        peer: WSETHgreenConnection = wallet_node.get_full_node_peer()
        if peer is None:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            ethgreen_tx = await self.create_tandem_xeth_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if ethgreen_tx is not None and ethgreen_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(ethgreen_tx)
