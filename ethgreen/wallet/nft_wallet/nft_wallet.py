...
from ethgreen.protocols.wallet_protocol import CoinState
...
from ethgreen.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from ethgreen.util.hash import std_hash
from ethgreen.util.ints import uint8, uint16, uint32, uint64, uint128
from ethgreen.wallet.derivation_record import DerivationRecord
from ethgreen.wallet.did_wallet import did_wallet_puzzles
from ethgreen.wallet.lineage_proof import LineageProof
from ethgreen.wallet.nft_wallet import nft_puzzles
from ethgreen.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from ethgreen.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from ethgreen.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from ethgreen.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from ethgreen.wallet.payment import Payment
from ethgreen.wallet.puzzle_drivers import PuzzleInfo, Solver
from ethgreen.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from ethgreen.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
from ethgreen.wallet.transaction_record import TransactionRecord
from ethgreen.wallet.uncurried_puzzle import uncurry_puzzle
from ethgreen.wallet.util.compute_memos import compute_memos
from ethgreen.wallet.util.debug_spend_bundle import disassemble
from ethgreen.wallet.util.transaction_type import TransactionType
from ethgreen.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
from ethgreen.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSETHgreenConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSETHgreenConnection) -> None:
    ...
    async def sign_message(self, message: str, nft: NFTCoinInfo) -> Tuple[G1Element, G2Element]:
        ...
        if uncurried_nft is not None:
            ...
            synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
            synthetic_pk = synthetic_secret_key.get_g1()
            puzzle: Program = Program.to(("ETHgreen Signed Message", message))
    ...
    async def create_tandem_xeth_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        ethgreen_coins = await self.standard_wallet.select_coins(fee)
        ethgreen_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
            (await self.standard_wallet.get_new_puzzlehash()),
            fee=fee,
            coins=ethgreen_coins,
            coin_announcements_to_consume={announcement_to_assert} if announcement_to_assert is not None else None,
        )
        assert ethgreen_tx.spend_bundle is not None
        return ethgreen_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, ethgreen_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if ethgreen_tx is not None and ethgreen_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, ethgreen_tx.spend_bundle])
            ethgreen_tx = dataclasses.replace(ethgreen_tx, spend_bundle=None)
        ...
        if ethgreen_tx is not None:
            tx_list.append(ethgreen_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            ethgreen_tx = await self.create_tandem_xeth_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, ethgreen_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XETH'} to offer")
    ...
    async def mint_from_did(
        ...
        xeth_coins: Optional[Set[Coin]] = None,
        xeth_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xeth spend that funds the transactions and pays fees.
        ...
        :param xeth_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xeth_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xeth spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xeth coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xeth_coins is None:
            xeth_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xeth_coins) > 0
        ...
        # Create the xeth spend to fund the minting.
        spend_value = sum([coin.amount for coin in xeth_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xeth_spends = []
        if xeth_change_ph is None:
            xeth_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xeth_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xeth_change_ph, "amount": change, "memos": [xeth_change_ph]})
        ]

        first = True
        for xeth_coin in xeth_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xeth_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xeth_coins]
                message_list.append(
                    Coin(xeth_coin.name(), xeth_primaries[0]["puzzlehash"], xeth_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xeth_coins) > 1:
                    xeth_announcement: Optional[Set[bytes]] = {message}
                else:
                    xeth_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xeth_primaries,
                    fee=fee,
                    coin_announcements=xeth_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xeth_coin.name(), message).name()
            ...
            xeth_spends.append(CoinSpend(xeth_coin, puzzle, solution))
        xeth_spend = await self.standard_wallet.sign_transaction(xeth_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xeth_spend, *eve_spends])
        return total_spend

    async def mint_from_xeth(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single XETH spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xeth_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XETH to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XETH spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XETH spend
            ...
            # Annnouncements for eve spend. These are asserted by the xeth spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xeth_coin in xeth_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xeth_primaries + primaries,
                    fee=fee,
                    coin_announcements=xeth_announcement if len(xeth_coins) > 1 else None,
...
if TYPE_CHECKING:
    from ethgreen.wallet.wallet_protocol import WalletProtocol
