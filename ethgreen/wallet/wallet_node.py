...
from ethgreen.consensus.blockchain import ReceiveBlockResult
from ethgreen.consensus.constants import ConsensusConstants
from ethgreen.daemon.keychain_proxy import (
    KeychainProxy,
    connect_to_keychain_and_validate,
    wrap_local_keychain,
)
from ethgreen.protocols import wallet_protocol
from ethgreen.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from ethgreen.rpc.rpc_server import default_get_connections
from ethgreen.server.node_discovery import WalletPeers
...
from ethgreen.types.weight_proof import WeightProof
from ethgreen.util.chunks import chunks
from ethgreen.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from ethgreen.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from ethgreen.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from ethgreen.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from ethgreen.wallet.util.wallet_sync_utils import (
...
from ethgreen.wallet.wallet_state_manager import WalletStateManager
from ethgreen.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    _server: Optional[ETHgreenServer] = None
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'ethgreen keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSETHgreenConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.eth-green.com"]),
    ...
    def on_disconnect(self, peer: WSETHgreenConnection):
    ...
    async def on_connect(self, peer: WSETHgreenConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def long_sync(
        ...
        full_node: WSETHgreenConnection,
        fork_height: int,
        *,
        rollback: bool,
    ):
    ...
    async def is_peer_synced(
        self, peer: WSETHgreenConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSETHgreenConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSETHgreenConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSETHgreenConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSETHgreenConnection]:
        """
        ...
        synced_and_trusted: List[WSETHgreenConnection] = []
        synced: List[WSETHgreenConnection] = []
        trusted: List[WSETHgreenConnection] = []
        neither: List[WSETHgreenConnection] = []
        all_nodes: List[WSETHgreenConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSETHgreenConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSETHgreenConnection):
        if self._wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSETHgreenConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSETHgreenConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSETHgreenConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSETHgreenConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSETHgreenConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSETHgreenConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
