...
p2_delegated_puzzle_or_hidden_puzzle is essentially the "standard coin" in ethgreen.
...
- The user's wallet contains a primary sk/pk pair which are used to derive to one
  ...
  https://ethgreenlisp.com/docs/standard_transaction. Therefore, when a wallet needs
...
- Python code previously appeared which was written like:
  ...
  In context, delegated_puzzle_solution here is any *ethgreenlisp program*, here one
  ...
  The second and third arguments are a ethgreenlisp program and its corresponding
...
from ethgreen.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
