...
from ethgreen.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from ethgreen.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            ethgreen_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        ethgreen_tx = dataclasses.replace(ethgreen_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(ethgreen_tx)
    ...
    async def create_tandem_xeth_tx(
        ...
        if fee > amount_to_claim:
            ethgreen_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(ethgreen_coins)[0].name()
            ...
            message = None
            for spend in ethgreen_tx.spend_bundle.coin_spends:
            ...
            ethgreen_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in ethgreen_coins])
            ethgreen_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return ethgreen_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XETH as part of this
        regular_ethgreen_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_ethgreen_to_claim = payment_amount

        need_ethgreen_transaction = (fee > 0 or regular_ethgreen_to_claim > 0) and (fee - regular_ethgreen_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_ethgreen_transaction:
                    if fee > regular_ethgreen_to_claim:
                        ethgreen_tx, _ = await self.create_tandem_xeth_tx(
                            fee,
                            uint64(regular_ethgreen_to_claim),
                    ...
                    elif regular_ethgreen_to_claim > fee:
                        ethgreen_tx, _ = await self.create_tandem_xeth_tx(
                            fee, uint64(regular_ethgreen_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        ethgreen_spend_bundle = SpendBundle([], G2Element())
        if ethgreen_tx is not None and ethgreen_tx.spend_bundle is not None:
            ethgreen_spend_bundle = ethgreen_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    ethgreen_spend_bundle,
                ]
            ),
            ethgreen_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        if ethgreen_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=ethgreen_tx.confirmed_at_height,
                    created_at_time=ethgreen_tx.created_at_time,
                    to_puzzle_hash=ethgreen_tx.to_puzzle_hash,
                    amount=ethgreen_tx.amount,
                    fee_amount=ethgreen_tx.fee_amount,
                    confirmed=ethgreen_tx.confirmed,
                    sent=ethgreen_tx.sent,
                    spend_bundle=None,
                    additions=ethgreen_tx.additions,
                    removals=ethgreen_tx.removals,
                    wallet_id=ethgreen_tx.wallet_id,
                    sent_to=ethgreen_tx.sent_to,
                    trade_id=ethgreen_tx.trade_id,
                    type=ethgreen_tx.type,
                    name=ethgreen_tx.name,
                    memos=[],
                )
            )
