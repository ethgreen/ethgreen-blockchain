...
from ethgreen.consensus.coinbase import farmer_parent_id, pool_parent_id
from ethgreen.consensus.constants import ConsensusConstants
from ethgreen.data_layer.data_layer_wallet import DataLayerWallet
from ethgreen.data_layer.dl_wallet_store import DataLayerStore
from ethgreen.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH, solution_to_pool_state
from ethgreen.pools.pool_wallet import PoolWallet
from ethgreen.protocols import wallet_protocol
...
from ethgreen.util.db_wrapper import DBWrapper2
...
from ethgreen.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from ethgreen.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from ethgreen.wallet.cat_wallet.cat_wallet import CATWallet
from ethgreen.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from ethgreen.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, create_fullpuz, match_did_puzzle
from ethgreen.wallet.key_val_store import KeyValStore
from ethgreen.wallet.nft_wallet.nft_info import NFTWalletInfo
from ethgreen.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from ethgreen.wallet.nft_wallet.nft_wallet import NFTWallet
from ethgreen.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from ethgreen.wallet.notification_manager import NotificationManager
from ethgreen.wallet.outer_puzzles import AssetType
from ethgreen.wallet.puzzle_drivers import PuzzleInfo
from ethgreen.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
from ethgreen.wallet.settings.user_settings import UserSettings
from ethgreen.wallet.trade_manager import TradeManager
from ethgreen.wallet.transaction_record import TransactionRecord
from ethgreen.wallet.util.address_type import AddressType
from ethgreen.wallet.util.compute_hints import compute_coin_hints
from ethgreen.wallet.util.transaction_type import TransactionType
from ethgreen.wallet.util.wallet_sync_utils import last_change_height_cs, PeerRequestException
from ethgreen.wallet.util.wallet_types import WalletType
from ethgreen.wallet.wallet import Wallet
from ethgreen.wallet.wallet_blockchain import WalletBlockchain
from ethgreen.wallet.wallet_coin_record import WalletCoinRecord
from ethgreen.wallet.wallet_coin_store import WalletCoinStore
from ethgreen.wallet.wallet_info import WalletInfo
from ethgreen.wallet.wallet_interested_store import WalletInterestedStore
from ethgreen.wallet.wallet_nft_store import WalletNftStore
from ethgreen.wallet.wallet_pool_store import WalletPoolStore
from ethgreen.wallet.wallet_puzzle_store import WalletPuzzleStore
from ethgreen.wallet.wallet_retry_store import WalletRetryStore
from ethgreen.wallet.wallet_transaction_store import WalletTransactionStore
from ethgreen.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSETHgreenConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xeth_spam_amount = self.config.get("xeth_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xeth_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xeth_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xeth_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSETHgreenConnection) -> Optional[bytes32]:
        # Get minter DID
