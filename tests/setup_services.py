...
from ethgreen.cmds.init_funcs import init
...
from ethgreen.protocols.shared_protocol import Capability, capabilities
from ethgreen.server.start_farmer import create_farmer_service
from ethgreen.server.start_full_node import create_full_node_service
from ethgreen.server.start_harvester import create_harvester_service
from ethgreen.server.start_introducer import create_introducer_service
from ethgreen.server.start_timelord import create_timelord_service
from ethgreen.server.start_wallet import create_wallet_service
from ethgreen.simulator.block_tools import BlockTools
from ethgreen.simulator.start_simulator import create_full_node_simulator_service
from ethgreen.timelord.timelord_launcher import kill_processes, spawn_process
...
from ethgreen.util.keychain import bytes_to_mnemonic
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["xeth_spam_amount"] = xeth_spam_amount
...
async def setup_farmer(
    ...
    service_config["xeth_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xeth")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["xeth_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xeth")
