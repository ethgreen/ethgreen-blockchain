...
from typing import Any, AsyncIterator, Dict, List, Tuple, Union
from ethgreen.server.start_service import Service

# Set spawn after stdlib imports, but before other imports
from ethgreen.clvm.spend_sim import SimClient, SpendSim
from ethgreen.full_node.full_node_api import FullNodeAPI
from ethgreen.protocols import full_node_protocol
from ethgreen.server.server import ETHgreenServer
from ethgreen.simulator.full_node_simulator import FullNodeSimulator
from ethgreen.simulator.simulator_protocol import FarmNewBlockProtocol

from ethgreen.types.peer_info import PeerInfo
from ethgreen.util.config import create_default_ethgreen_config, lock_and_load_config
...
from ethgreen.util.keyring_wrapper import KeyringWrapper
from ethgreen.simulator.block_tools import BlockTools, test_constants, create_block_tools, create_block_tools_async
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the ethgreen root directory
...
async def wallet_node_sim_and_wallet() -> AsyncIterator[
    Tuple[List[Union[FullNodeAPI, FullNodeSimulator]], List[Tuple[Wallet, ETHgreenServer]], BlockTools],
]:
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, xeth_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, xeth_spam_amount):
        yield _
...
@pytest_asyncio.fixture(scope="function")
async def one_node_one_block() -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], ETHgreenServer, BlockTools]]:
...
@pytest.fixture(scope="function")
def tmp_ethgreen_root(tmp_path):
    """
    Create a temp directory and populate it with an empty ethgreen_root directory.
    """
    path: Path = tmp_path / "ethgreen_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_ethgreen_root) -> Path:
    """
    Create a temp ethgreen_root directory and populate it with a default config.yaml.
    Returns the ethgreen_root path.
    """
    root_path: Path = tmp_ethgreen_root
