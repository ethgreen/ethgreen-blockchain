...
from ethgreen.protocols.harvester_protocol import PlotSyncIdentifier
from ethgreen.server.start_service import Service
from ethgreen.server.ws_connection import Message, NodeType
...
@dataclass
class WSETHgreenConnectionDummy:
...
def get_dummy_connection(node_type: NodeType, peer_id: Optional[bytes32] = None) -> WSETHgreenConnectionDummy:
    return WSETHgreenConnectionDummy(node_type, bytes32(token_bytes(32)) if peer_id is None else peer_id)
