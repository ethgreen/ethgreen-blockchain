...
from ethgreen.data_layer.data_layer_util import NodeType, Status
from ethgreen.data_layer.data_store import DataStore
from ethgreen.types.blockchain_format.tree_hash import bytes32
...
@pytest.fixture(name="ethgreen_root", scope="function")
def ethgreen_root_fixture(tmp_path: pathlib.Path, scripts_path: pathlib.Path) -> ETHgreenRoot:
    root = ETHgreenRoot(path=tmp_path.joinpath("ethgreen_root"), scripts_path=scripts_path)
    root.run(args=["init"])
...
def closing_ethgreen_root_popen(ethgreen_root: ETHgreenRoot, args: List[str]) -> Iterator[None]:
    environment = {**os.environ, "ETHGREEN_ROOT": os.fspath(ethgreen_root.path)}
...
@pytest.fixture(name="ethgreen_daemon", scope="function")
def ethgreen_daemon_fixture(ethgreen_root: ETHgreenRoot) -> Iterator[None]:
    with closing_ethgreen_root_popen(ethgreen_root=ethgreen_root, args=[sys.executable, "-m", "ethgreen.daemon.server"]):
...
@pytest.fixture(name="ethgreen_data", scope="function")
def ethgreen_data_fixture(ethgreen_root: ETHgreenRoot, ethgreen_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_ethgreen_root_popen(ethgreen_root=ethgreen_root, args=[os.fspath(scripts_path.joinpath("ethgreen_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
