...
from tests.core.data_layer.util import ETHgreenRoot

pytestmark = pytest.mark.data_layer


@pytest.mark.asyncio
async def test_help(ethgreen_root: ETHgreenRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = ethgreen_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.xfail(strict=True)
@pytest.mark.asyncio
def test_round_trip(ethgreen_root: ETHgreenRoot, ethgreen_daemon: None, ethgreen_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with ethgreen_root.print_log_after():
        create = ethgreen_root.run(args=["data", "create_data_store"])
