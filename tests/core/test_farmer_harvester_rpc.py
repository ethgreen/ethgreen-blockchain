...
from ethgreen.protocols import farmer_protocol
from ethgreen.protocols.harvester_protocol import Plot
from ethgreen.rpc.farmer_rpc_api import (
    FilterItem,
...
from ethgreen.rpc.farmer_rpc_client import FarmerRpcClient
from ethgreen.rpc.harvester_rpc_client import HarvesterRpcClient
from ethgreen.simulator.block_tools import get_plot_dir
from ethgreen.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from ethgreen.util.config import load_config, lock_and_load_config, save_config
from ethgreen.util.hash import std_hash
from ethgreen.util.ints import uint8, uint32, uint64
from ethgreen.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xeth"), encode_puzzle_hash(new_ph_2, "xeth"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xeth"), encode_puzzle_hash(observer_pool, "xeth")
    )
    ...
    assert config["farmer"]["xeth_target_address"] == encode_puzzle_hash(observer_farmer, "xeth")
    assert config["pool"]["xeth_target_address"] == encode_puzzle_hash(observer_pool, "xeth")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xeth")
