...
from ethgreen.wallet.cat_wallet.cat_utils import (
...
from ethgreen.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XETH coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            ethgreen_coins: List[Coin] = all_coins[None]
            ...
            # Create an XETH Offer for RED
            ethgreen_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            ethgreen_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                ethgreen_requested_payments, ethgreen_coins
            )
            ethgreen_announcements: List[Announcement] = Offer.calculate_announcements(ethgreen_requested_payments, driver_dict)
            ethgreen_secured_bundle: SpendBundle = generate_secure_bundle(ethgreen_coins, ethgreen_announcements, 1000)
            ethgreen_offer = Offer(ethgreen_requested_payments, ethgreen_secured_bundle, driver_dict)
            assert not ethgreen_offer.is_valid()

            # Create a RED Offer for XETH
            ...
            new_offer = Offer.aggregate([ethgreen_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for XETH and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xeth": 900, str_to_tail_hash("red").hex(): 350},
