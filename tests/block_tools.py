...
from ethgreen.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from ethgreen.plotting.create_plots import create_plots, PlotKeys
...
from ethgreen.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from ethgreen.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
from ethgreen.plotting.manager import PlotManager
from ethgreen.server.server import ssl_context_for_server
...
from ethgreen.util.config import load_config, save_config
...
from ethgreen.util.merkle_set import MerkleSet
from ethgreen.util.prev_transaction_block import get_prev_transaction_block
from ethgreen.util.path import mkdir
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="ethgreen-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `ethgreen generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("ETHGREEN_ROOT", "~/.ethgreen/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
