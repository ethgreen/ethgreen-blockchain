...
from ethgreen.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
from ethgreen.simulator.simulator_test_tools import get_full_ethgreen_simulator, get_puzzle_hash_from_key
...
class TestStartSimulator:
    ...
    async def get_ethgreen_simulator(
        self, automated_testing: bool = False, ethgreen_root: Optional[Path] = None, config: Optional[Dict[str, Any]] = None
    ) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int], None]:
        async for simulator_args in get_full_ethgreen_simulator(automated_testing, ethgreen_root, config):
            yield simulator_args

    @pytest.mark.asyncio
    async def test_start_simulator(
        self, get_ethgreen_simulator: Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int]
    ) -> None:
        simulator, root_path, config, mnemonic, fingerprint = get_ethgreen_simulator
