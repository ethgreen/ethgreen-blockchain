...
from ethgreen.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
...
from ethgreen.wallet.derive_keys import find_authentication_sk, find_owner_sk
from ethgreen.wallet.transaction_record import TransactionRecord
from ethgreen.wallet.util.wallet_types import WalletType
from ethgreen.simulator.block_tools import BlockTools, get_plot_dir
...
class TestPoolWalletRpc:
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc, trusted_and_fee, self_hostname):
        ...
        async with TemporaryPoolPlot(bt, status.p2_singleton_puzzle_hash) as pool_plot:
            ...
            tr: TransactionRecord = await client.send_transaction(
                1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "txeth")
            )
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc, trusted_and_fee, self_hostname):
        ...
        async with TemporaryPoolPlot(bt, status.p2_singleton_puzzle_hash) as pool_plot:
            ...
            # Balance stars at 6 XETH and 5 more blocks are farmed, total 22 XETH
    ...
    async def test_leave_pool(self, setup, trusted_and_fee, self_hostname):
        ...
        try:
            ...
            async def have_ethgreen():
                return (await wallets[0].get_confirmed_balance()) > FEE_AMOUNT

            await time_out_assert(timeout=MAX_WAIT_SECS, function=have_ethgreen)
    ...
    async def test_change_pools(self, setup, trusted_and_fee, self_hostname):
        ...
        try:
            ...
            await time_out_assert(timeout=WAIT_SECS, function=have_ethgreen)
            await time_out_assert(20, wallet_is_synced, True, wallet_nodes[0], full_node_api)
