...
# Gets the version from the result of "ethgreen version"
...
def get_ethgreen_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["ethgreen", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_ethgreen_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/ethgreen-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/ethgreen-blockchain-gui/packages/gui/package.json")
