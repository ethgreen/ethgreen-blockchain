...
In the event an emergency fix is required for the release version of ethgreen, members of the ethgreen organization will create a feature branch from the current release branch `1.0.0`.
...
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the ethgreen blockchain.
...
If you are not a ethgreen organization member, you can enquire about deploying a `beta testnet` in the public dev Keybase channel.
