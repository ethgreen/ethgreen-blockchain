...
import { AlertDialog, Card, Flex, ethgreenToMojo, mojoToETHgreenLocaleString } from '@ethgreen/core';
import {
  send_transaction,
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  ...
                  {mojoToETHgreenLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToETHgreenLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = ethgreenToMojo(fee_input.value);
