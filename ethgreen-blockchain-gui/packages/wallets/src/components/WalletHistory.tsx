...
import type { Row } from '@ethgreen/core';
import { WalletType, TransactionType, toBech32m } from '@ethgreen/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToETHgreen(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
