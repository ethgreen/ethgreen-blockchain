...
import { useSpendCATMutation, useFarmBlockMutation } from '@ethgreen/api-react';
import { SyncingStatus, toBech32m } from '@ethgreen/api';
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(
        t`Cannot send ethgreen to coloured address. Please enter a ethgreen address.`
      );
    }
    ...
    const feeValue = ethgreenToMojo(fee);
