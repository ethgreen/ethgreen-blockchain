...
export default function ProfileAdd() {
  ...
  async function handleSubmit(data: CreateProfileData) {
    ...
    const walletId = await createProfile({
      ...
      options: {did_type: 'new', backup_dids: [], num_of_backup_ids_needed: '0', amount: 1, fee: ethgreenToMojo(fee)},
    }).unwrap();

    navigate(`/dashboard/settings/profiles/${walletId}`);
  }

  const standardBalance = mojoToETHgreenLocaleString(balance?.confirmedWalletBalance);
        ...
        <StyledCard>
          ...
          <div style={{cursor: "pointer"}}>
            <Flex paddingBottom={5}>
              <Typography onClick={handleClick} sx={{ textDecoration: "underline" }}>Get Mojos from the ETHgreen Faucet</Typography>
            </Flex>
          </div>
          <Flex flexDirection="column" gap={2.5} paddingBottom={1}>
