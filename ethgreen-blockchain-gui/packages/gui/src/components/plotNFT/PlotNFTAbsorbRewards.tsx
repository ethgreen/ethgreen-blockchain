...
import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@ethgreen/api-react'
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const { fee } = data;
      const feeMojos = ethgreenToMojo(fee);
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={mojoToETHgreenLocaleString(balance)}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {address}
