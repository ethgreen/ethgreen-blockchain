...
import type { CalculateRoyaltiesRequest, NFTInfo } from '@ethgreen/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@ethgreen/api-react';
import { catToMojo, ethgreenToMojo } from '@ethgreen/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const fungibleAssets = [
    ...(fungibleXCH ?? [])
      ...
      .map(({ amount }) =>
        fungibleAssetFromAssetIdAndAmount('xfx', ethgreenToMojo(amount)),
      ),
    ...(fungibleTokens ?? [])
