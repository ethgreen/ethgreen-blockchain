...
export function formatAmountForWalletType(
  ...
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToETHgreenLocaleString(amount, locale);
...
export function offerAssetIdForAssetType(
  ...
  if (assetType === OfferAsset.ETHGREEN) {
    return 'xfx';
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.ETHGREEN]) {
