...
import { OfferSummaryRecord, OfferTradeRecord } from '@ethgreen/api';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.ETHGREEN: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xfxMakerFee = mojoToETHgreen(makerFee);
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.ETHGREEN
        ? ethgreenToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
