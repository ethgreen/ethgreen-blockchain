...
export function getUserDataDir(): string {
  const ethgreenRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(ethgreenRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const ethgreenRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${ethgreenRootUserDataPath}`);
  app.setPath('userData', ethgreenRootUserDataPath);
}
