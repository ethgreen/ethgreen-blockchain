...
export default function Block() {
  ...
  const poolReward = mojo_to_ethgreen(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_ethgreen(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const ethgreenFees = blockRecord.fees
    ? mojo_to_ethgreen(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.ethgreenexplorer.org/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.ethgreenexplorer.org/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: ethgreenFees ? `${ethgreenFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Ethgreen blockchain
            </Trans>
          </Back>
        }
        action={
