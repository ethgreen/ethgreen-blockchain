...
import { AlertDialog, Card, Flex } from '@ethgreen/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const ethgreenper_input = ip_parsed.limit;
    ...
    const ethgreenper_value = Number.parseInt(Number(ethgreenper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (ethgreen per interval):{' '}
                  {mojo_to_ethgreen_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = ethgreen_to_mojo(fee_input.value);
