...
import { AlertDialog } from '@ethgreen/core';
import {
  Typography,
  Button,
  Box,
...
import { ethgreen_to_mojo } from '../../../util/ethgreen';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      ethgreenper_input.value === '' ||
      Number(ethgreenper_input.value) === 0 ||
      !Number(ethgreenper_input.value) ||
      isNaN(Number(ethgreenper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const ethgreenper = ethgreen_to_mojo(ethgreenper_input.value);
    const ethgreenper_value = Number.parseInt(Number(ethgreenper));
    const userpubkey = userpubkey_input.value;
    const amount = ethgreen_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = ethgreen_to_mojo(fee_input.value);
