...
get_madmax_filename() {
  ...
  ETHGREEN_PLOT="ethgreen_plot"
  if [ "$KSIZE" = "k34" ]; then
    ETHGREEN_PLOT="ethgreen_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
  ...
  echo "${ETHGREEN_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the ethgreen python virtual environment."
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" ethgreen_plot
  chmod 755 ethgreen_plot
  echo "Successfully installed madmax to $(pwd)/ethgreen_plot"
  ...
  mv -f "${madmax_filename}" ethgreen_plot_k34
  chmod 755 ethgreen_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/ethgreen_plot_k34"
else
  usage
fi
